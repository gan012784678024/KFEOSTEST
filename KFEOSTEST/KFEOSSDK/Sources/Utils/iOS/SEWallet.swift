...
@objcMembers class SEKeystoreService: NSObject {
    ...
    func createRandomKeyPair() -> (Dictionary<String, Any>) {
        let privateKey = PrivateKey.randomPrivateKey(enclave: .Secp256k1)
        let publicKey = PublicKey(privateKey: privateKey!)
        
        return ["privateKey": privateKey?.rawPrivateKey() ?? "", "publicKey": publicKey.rawPublicKey() ]
    }
